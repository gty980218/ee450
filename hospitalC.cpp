...
void ready(int sock,Hospital& hospital){
    ...
    cout<<"Hospital C is up and running using UDP on port 32188"<<endl;
    ...
    while(1){
        ...
        if(n==-1){
            ...
            if(rev_str.compare("assigned")==0){                                 //assigned response
                ...
                cout<<"Hospital C has been assigned to a client, occupancy updated to "<<hospital.occupancy<<", availability is updated to "<<avil<<endl;
            }else{                                                              //availability response
                int loc=atoi(buff);
                cout<<"Hospital C has received input from client at location "<<loc<<endl;
                double dist=-1;
                double avil=0;
                double score=0;
                if(loc!=hospital.location){
                    ...
                    if(iter!=indice_map.end()){                                     //loc exits
                        ...
                        cout<<"Hospital C has capacity = "<<hospital.capacity<<" ,occupancy= "<<hospital.occupancy<<" , availability = "<<avil<<endl;
                        cout<<"Hospital C has found the shortest path to client,distance = "<<dist<<endl;
                        cout<<"Hospital C has the score = "<<score<<endl;
                    }else{
                        cout<<"Hospital C does not have the location "<<loc<<" in map"<<endl;
                ...
                if(dist==-1 || avil==-1){
                    ...
                    string str=ss.str();
                    strcpy(buff,str.c_str());
                    cout<<"Hospital C has sent score = "<<score<<" and distance= "<<dist<<" to the Scheduler"<<endl;
                }
